...
export function FilterBuilder({
  ...
  useEffect(() => {
    void Promise.all([
      api.get('/services').then((r) => setServices(r.data?.data ?? r.data ?? [])),
      api.get('/appointments').then((r) => setAppointments(r.data?.data ?? r.data ?? [])),
      api.get('/data-query/staff').then((r) => setStaff(r.data ?? [])),
    ]).catch(() => undefined);
  }, []);

  const updateChild = (patch: Partial<NonNullable<DataQueryFilters['child']>>) => {
    onFiltersChange({
      ...filters,
  ...
  const updateParent = (patch: Partial<NonNullable<DataQueryFilters['parent']>>) => {
    onFiltersChange({
      ...filters,
  ...
  const updateLocation = (patch: Partial<NonNullable<DataQueryFilters['location']>>) => {
    onFiltersChange({
      ...filters,
  ...
  const updateServices = (patch: Partial<NonNullable<DataQueryFilters['services']>>) => {
    onFiltersChange({
      ...filters,
  ...
  const updateTraining = (patch: Partial<NonNullable<DataQueryFilters['training']>>) => {
    onFiltersChange({
      ...filters,
  ...
  const updateFinancial = (patch: Partial<NonNullable<DataQueryFilters['financial']>>) => {
    onFiltersChange({
      ...filters,
  ...
  const updateProgress = (patch: Partial<NonNullable<DataQueryFilters['progress']>>) => {
    onFiltersChange({
      ...filters,
  ...
  const updateCaseWorker = (patch: Partial<NonNullable<DataQueryFilters['caseWorker']>>) => {
    onFiltersChange({
      ...filters,
  ...
  const subjects: { value: DataSubject; label: string }[] = [
    ...
    { value: 'PARENT_CHILD_PAIR', label: t('dataQuery.pairs', 'Parent-Child Pairs') },
  ];
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-4">
          <div>
            <Label className="mb-2 block">{t('dataQuery.showResultsFor', 'Show results for')}</Label>
          ...
          <Section
            ...
            badge={countSectionFilters(filters.location as Record<string, unknown>)}
          >
            <Input
              placeholder="City"
              value={filters.location?.city ?? ''}
              onChange={(e) => updateLocation({ city: e.target.value || undefined })}
            />
            <MultiCheckbox
          ...
          <Section
            ...
            badge={countSectionFilters(filters.services as Record<string, unknown>)}
          >
          ...
          <Section
            ...
            badge={countSectionFilters(filters.training as Record<string, unknown>)}
          >
            <label className="flex items-center gap-2 text-sm">
              <Checkbox
          ...
          <Section
            ...
            badge={countSectionFilters(filters.financial as Record<string, unknown>)}
          >
            <MultiCheckbox
              options={[
          ...
          <Section
            ...
            badge={countSectionFilters(filters.progress as Record<string, unknown>)}
          >
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  updateProgress({
                    noNoteInLastDays: e.target.value ? Number(e.target.value) : undefined,
                  })
                }
              />
            </div>
          ...
          <Section
            ...
            badge={countSectionFilters(filters.caseWorker as Record<string, unknown>)}
          >
            <select
              className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm disabled:opacity-50"
              disabled={filters.caseWorker?.unassigned}
              value={filters.caseWorker?.staffId ?? ''}
              onChange={(e) => updateCaseWorker({ staffId: e.target.value || undefined })}
            >
          ...
          <div className="rounded-md border p-3">
            ...
            <label className="mt-3 flex items-center gap-2 text-sm">
              <Checkbox checked={anonymize} onCheckedChange={(c) => onAnonymizeChange(Boolean(c))} />
